 ...
 */
@RestController
@RequestMapping(value = "/contract/crmContract")
public class CrmContractController extends BaseController {
    ...
    @GetMapping("/byotyid/{id}")
    public ResponseEntity<JSONObject> byotyid(@PathVariable("id") String id) {
        CrmContract entity = crmContractService.byotyid(id);
        crmContractService.entityToDto(entity, true);
        return ResponseEntity.ok(ResultUtil.successJson(entity));
    }
...
//    public ResponseEntity<JSONObject> postsave(@RequestParam(value = "crmContract", required = false) String strEntity,  @RequestParam(value = "attachments", required = false)MultipartFile[] attachments) {
