...
@Service("crmCustomerContractService")
@Transactional(readOnly = false)
public class CrmCustomerContractService extends BizInProcService<CrmBusunessContractDao, CrmContract> {
    @Autowired
...
//    private CrmOpportunityCotService opportunityCotService;
    private CrmCustomerCotService customerCotService;
    ...
    public int saveDraft(WfDraft wfDraft, CrmContract contract, MultipartFile[] attachments) throws IOException {
        ...
        if (StringUtils.isNotBlank(contract.getOpporStatus())){
            ...
            customerCotService.saveContract(crmContract);
        }
        saveAttachment(attachments,crmContract);
    ...
    public void savePayList(CrmContract crmContract) {
        ...
        // 生成商机回款记录
        List<CrmCustomerPay> list = new ArrayList<>();
        listPayIds.forEach(items->{
            CrmCustomerPay pay = new CrmCustomerPay();
            pay.setCusId(crmContract.getCustomerId());
            pay.setPayId(items);
            list.add(pay);
        });
        crmCustomerPayService.bulkInsert(list);
    }
