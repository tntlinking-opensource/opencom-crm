...
export default {
  ...
  methods: {
    getList() {
      ...
      listCrmMyclueProductsPage(this.search)
        .then((responseData) => {
        ...
        .then((responseData) => {
          if (responseData.code == 100) {
            this.permission.view = responseData.data.find((item) => {
              return item.permission === 'crmMyclueProducts:read'
            })
            this.permission.export = responseData.data.find((item) => {
              return item.permission === 'crmMyclueProducts:export'
            })

            this.permission.add = responseData.data.find((item) => {
              return item.permission === 'crmMyclueProducts:create'
            })
            this.permission.edit = responseData.data.find((item) => {
              return item.permission === 'crmMyclueProducts:update'
            })
            this.permission.remove = responseData.data.find((item) => {
              return item.permission === 'crmMyclueProducts:delete'
            })
          } else {
            this.showMessage(responseData)
          }
