<template>
  <el-dialog v-on:open="onDialogOpen" v-on:close="onDialogClose" v-loading="loading" width="88%" :fullscreen="false"
    ...
    <el-form ref="taxDetailsInfoForm" :model="editFormData" label-width="100px" :disabled="action == 'view'"
      ...
      <el-row>
        <ux-grid :data="editFormData.paymentBackList" ref="taxDetailsInfoTable" :currentRow="table124CurrentRow"
          ...
          ></ux-table-column>
          <ux-table-column field="id" title="id" tree-node resizable min-width="60px" align="center"
                           header-align="center" v-if="false" show-overflow
                           class="dc-ux-table-column_ElTableColumn">
          </ux-table-column>
          <ux-table-column field="order" title="期数" tree-node resizable min-width="60px" align="center"
                           header-align="center" show-overflow
                           class="dc-ux-table-column_ElTableColumn">
          </ux-table-column>
          <ux-table-column field="payDate" title="计划还款日期" tree-node resizable min-width="120px" align="center"
                           header-align="center" show-overflow
                           class="dc-ux-table-column_ElTableColumn">
          </ux-table-column>
          <ux-table-column field="payRate" title="计划还款比例" tree-node resizable min-width="120px" align="center"
                           header-align="center" show-overflow
                           class="dc-ux-table-column_ElTableColumn">
                           <template slot-scope="{ row }">
                            <div class="display: flex; justify-content: end;width:100%;">{{ row.payRate }}%</div>
                           </template>
          </ux-table-column>
          <ux-table-column field="payAmount" title="计划还款金额" tree-node resizable width="120px" align="right"
                           header-align="center" show-overflow
                           class="dc-ux-table-column_ElTableColumn">
                           <template slot-scope="{ row }">
                            <div style="display: flex; justify-content: end;width:120px!important;">￥{{ row.payAmount }}</div>
                           </template>
          </ux-table-column>
          <ux-table-column field="paymentBackDate" title="实际回款日期" tree-node resizable min-width="120px"
            ...
            <template v-slot:edit="{ row, rowIndex }">
              <el-form-item :prop="'paymentBackList.' + rowIndex + '.paymentBackDate'" label-width="0px"
                ...
                <el-date-picker v-model="row.paymentBackDate" :style="{ width: '100%' }" :disabled="action === 'view'" placeholder="请选择"
                                type="date"
            ...
            <template v-slot:edit="{ row, rowIndex }">
              <el-form-item :prop="'paymentBackList.' + rowIndex + '.paymentBackAmount'" label-width="0px"
                ...
                <el-input v-model="row.paymentBackAmount" placeholder="请输入实际回款金额" clearable
                          autocomplete="new-password" :disabled="action === 'view'"
          ...
          <ux-table-column field="remark" title="备注" tree-node resizable min-width="120px" align="alter"
            ...
            <template v-slot:edit="{ row, rowIndex }">
              <el-form-item :prop="'paymentBackList.' + rowIndex + '.remark'" label-width="0px"
                ...
                :disabled="action === 'view'" class="dc-el-input_SingleInput"></el-input>
              </el-form-item>
            </template>
            <template v-slot="{ row, rowIndex }">
                  <span class="my-input-sc">
...
import { getCrmCustomerPayById, saveCrmCustomerPayBacks } from '@/api/crmCustomer/crmCustomerPay'
...
export default {
  ...
  methods: {
    ...
    payListValid(){
      ...
      let payList = this.editFormData.paymentBackList // 列表
      let payRate = 0;
      let payAmount = 0;
      ...
      for (let i = 0; i < payList.length; i++) {
        payRate += payList[i].payRate // 还款比例
        payAmount += payList[i].paymentBackAmount // 还款总额是否等于合同金额
        if (payAmount > this.editFormData.contractAmount) {
          this.$message.error("还款金额大于合同金额，请核实。");
          return false;
        } else if (payList.length < i+1) {
          if (payList[i].paymentBackDate > payList[i+1].paymentBackDate) {
    ...
    doSave() {
      if (this.editFormData.paymentBackList.length > 0) {
        ...
        if (!isEmpty && isValid) {
          ...
          this.editFormData.paymentBackList[0].contractId = this.editFormData.contractId
          saveCrmCustomerPayBacks(this.editFormData.paymentBackList)
            .then((responseData) => {
