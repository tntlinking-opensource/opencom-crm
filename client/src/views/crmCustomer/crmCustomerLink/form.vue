...
import {treeCrmCustomer, listCrmCustomerPage} from "@/api/crmCustomer/crmCustomer";
import {treeDepartment} from "@/api/org/department";
import {listCrmOpportunityLinkPage} from "@/api/crmBusiness/crmOpportunityLink";
import {saveCrmtacts} from '@/api/crmCustomer/crmCustomerLink';
...
export default {
  ...
  methods: {
    ...
    listSelectOptions1() {
      ...
      listCrmCustomerPage(search_List).then(responseData => {
        if (responseData.code == 100) {
          this.selectOptions1 = responseData.data.rows || []
        } else {
          this.showMessage(responseData)
        }
      })
    ...
    listSelectOptions5() {
      let search_List = {
        params: [{'columnName': 'cus_id', 'queryType': '=', 'value': this.editFormData.customName}]
      }
      listCrmCustomerOppPage(search_List).then((responseData) => {
        if (responseData.code == 100) {
          this.selectOptions5 = responseData.data.rows;
        } else {
          this.showMessage(responseData)
        }
      })
    ...
    this.$nextTick(() => {
      ...
      this.$on('openEditDialog', async function (row) {
        ...
        this.editFormData = {
          ...
          ...row.link,
          id: row.link.id,
          bsoy: row.link.bsoyid
        }
        // this.listSelectOptions2(row.cus.id)
        this.editFormData.customName = row.cus.id;
        this.initOptions(this.editFormData)
        this.dialogProps.visible = true
      })
