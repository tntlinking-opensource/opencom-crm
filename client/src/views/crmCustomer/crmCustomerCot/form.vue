...
<!--                title="文件上传"-->
<!--                :action="action"-->
<!--                :objectId="editFormData.null"-->
<!--                v-model="editFormData.nullFile"-->
<!--                class="dc-upload-file_ElUploadFile"-->
<!--              ></upload-file>-->
<!--            </el-col>-->
<!--            <el-col :span="24">-->
<!--              <el-form-item prop="null" class="dc-el-form-item_ElCheckbox">-->
<!--                <el-checkbox v-model="editFormData.null" class="dc-el-checkbox_ElCheckbox">是否为下次跟进创建任务</el-checkbox>-->
<!--              </el-form-item>-->
<!--            </el-col>-->
<!--            <el-col :span="12">-->
...
<!--            </el-col>-->
<!--            <el-col :span="6">-->
<!--              <el-form-item prop="null" label="标题" class="dc-el-form-item_CounterInput">-->
<!--                <el-input-number-->
<!--                  v-model="editFormData.null"-->
<!--                  :min="0"-->
<!--                  :max="999"-->
<!--                  :step="1"-->
<!--                  :precision="0"-->
...
<!--/** 根据用户界面配置import组件 结束 */-->
<!--import { getCrmCustomerCotById, saveCrmCustomerCot } from '@/api/crmCustomer/crmCustomerCot'-->
<!--import BaseUI from '@/views/components/baseUI'-->
<!--import OperationIcon from '@/components/OperationIcon'-->
<!--export default {-->
<!--  extends: BaseUI,-->
<!--  name: 'CrmCustomerCot-form',-->
<!--  components: {-->
<!--    /** 根据用户界面配置组件 开始 */-->
<!--    uploadFile,-->
<!--    /** 根据用户界面配置组件 结束 */-->
...
<!--    doSave() {-->
<!--      this.setLoad()-->
<!--      saveCrmCustomerCot(this.editFormData)-->
<!--        .then((responseData) => {-->
        ...
        <el-form ref="editFormDialog" :model="editFormData" label-width="100px" :disabled="action == 'view'"
          ...
          <el-row>
            ...
            <el-col :span="8">
              ...
              >
                <el-select
                  v-model="editFormData.opportunityId"
                  :style="{ width: '100%' }"
                  placeholder="请选择商机"
                  clearable
                  filterable
                ...
                >
                  <el-option
                    v-for="(item, index) in oppOptions"
                    :key="index"
                    :label="item.oppName"
                    :value="item.id"
                  ></el-option>
            ...
            <el-col :span="8">
              <el-form-item prop="contractName" label="合同金额" class="dc-el-form-item_SingleInput">
                <el-input v-model="editFormData.contractAmount" :maxLength="255" placeholder="请输入合同金额" clearable
                          autocomplete="new-password" :disabled="true" :value="this.totalPrice"
    ...
    <el-dialog v-on:close="onTableDialogClose"
      ...
      <el-table :data="tableData2"
        ...
        <el-table-column prop="pdPriceSum" label="总价">
          <template slot-scope="scope">
            {{
            scope.row.pdPriceSum = scope.row.sigleNum && scope.row.discount ? scope.row.pdGuidePrice *
            scope.row.sigleNum
...
import {saveCrmContractPost} from "@/api/contract/crmContract";
import {listCrmOpportunityAll} from "@/api/crmBusiness/crmOpportunity.js";
...
export default {
  ...
  methods: {
    ...
    listSelectOptions2(id) {
      listCrmOpportunityAll({
        customer_id:id
      }).then((responseData) => {
        if (responseData.code == 100) {
          this.editFormData.oppName = responseData.data[1].oppName;
          this.editFormData.opportunityId = responseData.data[1].id;
          this.oppOptions = responseData.data;
        } else {
          this.showMessage(responseData)
        }
      })
  ...
  mounted: function () {
    this.$nextTick(() => {
      ...
      this.$on('openEditDialog', async function (conId) {
        this.action = 'edit'
      ...
      this.$on('openAddDialog', function (customerId) {
        this.action = 'add'
        this.dialogProps.title = `添加${this.dialogTitle}`
        this.editFormData = this.initEditData()
        ...
        // this.totalPrice = totalRepayment;
        // this.editFormData.contractAmount = totalRepayment
        this.editFormData.customerId = customerId
        // debugger
        this.listSelectOptions2(customerId)
        this.initOptions(this.editFormData)
        this.dialogProps.visible = true
      })
      this.$on('openCopyDialog', async function (id) {
