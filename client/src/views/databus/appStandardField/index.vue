...
import { listAppStandardFieldPage, deleteAppStandardField } from '@/api/databus/appStandardField'
...
export default {
  ...
  methods: {
    getList() {
      ...
      // 数据权限: app应用主数据字段app_standard_field
      this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
      listAppStandardFieldPage(this.search)
        .then((responseData) => {
    ...
    async pageInit() {
      ...
      try {
        ...
        // 数据权限: app应用主数据字段app_standard_field
        this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
        let [listAppStandardFieldRespData, listPermissionRespData] = await Promise.all([
          listAppStandardFieldPage(this.search),
          listResourcePermission(this.$route.meta.routerId)
        ])
        if (listAppStandardFieldRespData.code == 100 && listPermissionRespData.code == 100) {
          this.tableDataTotal = listAppStandardFieldRespData.data.total
          this.tableData = listAppStandardFieldRespData.data.rows ? listAppStandardFieldRespData.data.rows : []
          this.permission.view = listPermissionRespData.data.find((item) => {
            return item.permission === 'appStandardField:read'
          })
          this.permission.export = listPermissionRespData.data.find((item) => {
            return item.permission === 'appStandardField:export'
          })

          this.permission.add = listPermissionRespData.data.find((item) => {
            return item.permission === 'appStandardField:create'
          })
          this.permission.edit = listPermissionRespData.data.find((item) => {
            return item.permission === 'appStandardField:update'
          })
          this.permission.remove = listPermissionRespData.data.find((item) => {
            return item.permission === 'appStandardField:delete'
          })
        } else {
          this.showMessage(listPermissionRespData.code != 100 ? listPermissionRespData : listAppStandardFieldRespData)
        }
        this.resetLoad()
      } catch (error) {
        this.outputError(error)
    ...
    onDelete(scope) {
      this.$confirm('确定删除吗？', '确认', {
        ...
        .then(() => {
          this.setLoad()
          deleteAppStandardField(scope.row)
            .then((responseData) => {
              if (responseData.code == 100) {
                this.getList()
                this.showMessage({
