<template>
  <el-row class="dc-container">
    <push-interface-index
      ref="PushInterfaceIndex"
      :permission="PushInterfaceIndexpermission"
    ></push-interface-index>
    <el-dialog
    ...
    >
      <el-row gutter="0" type="flex" justify="start" align="top" class="dc-el-row_ElRow">
        <el-tooltip  disabled class="dc-el-tooltip_Button">
          <el-button type="primary" icon="el-icon-info" v-on:click="onView">查看接口推送记录	</el-button>
        </el-tooltip>
      </el-row>
      <ux-grid
        column-key
      ...
      >
        ...
        >
          <template slot-scope='{row, rowIndex}'>
            ...
            return row.targetApp.app.name+row.targetApp.name
            }
            )(row.isSystem, row, rowIndex)'>
...
import PushInterfaceIndex from './pushinterfaceform'
/** 根据用户界面配置import组件 结束 */
...
export default {
  ...
  methods: {
    getList() {
      ...
      listMasterDataPushRecordsPage(this.search)
        .then((responseData) => {
