<template>
  <el-row class="dc-container" v-loading="loading">
    ...
    <push-index
      ref="PushIndex"
      :permission="PushIndexpermission"
    ></push-index>
    ...
    <el-row gutter="0" type="flex" justify="start" align="top" class="dc-el-row_ElRow">
      <el-tooltip v-show="permission.add" disabled class="dc-el-tooltip_Button">
        <el-button type="primary" icon="el-icon-info" v-on:click="onJsonView">查看原始json</el-button>
      </el-tooltip>
      <el-tooltip v-show="permission.add" disabled class="dc-el-tooltip_Button">
        <el-button type="primary" icon="el-icon-info" v-on:click="onView">查看分发记录</el-button>
      </el-tooltip>
    </el-row>
    <ux-grid
      column-key
...
import { listMasterDataBatchRecordsPage, deleteMasterDataBatchRecords } from '@/api/databus/masterDataBatchRecords'
...
import PushIndex from './pushform'
import JsonView from '@/views/databus/masterDataJson/form'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [],
      tableId: '1562488744152309763',
      schemeId: '1562488744152309761'
    }
  },
  computed: {},
  methods: {
    getList() {
      ...
      // 数据权限: 主数据推送查看master_data_batch_records
      this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
      listMasterDataBatchRecordsPage(this.search)
        .then((responseData) => {
        ...
        .then((responseData) => {
          if (responseData.code == 100) {
            this.permission.view = responseData.data.find((item) => {
              return item.permission === 'masterDataBatchRecords:read'
            })
            this.permission.export = responseData.data.find((item) => {
              return item.permission === 'masterDataBatchRecords:export'
            })

            this.permission.add = responseData.data.find((item) => {
              return item.permission === 'masterDataBatchRecords:create'
            })
            this.permission.edit = responseData.data.find((item) => {
              return item.permission === 'masterDataBatchRecords:update'
            })
            this.permission.remove = responseData.data.find((item) => {
              return item.permission === 'masterDataBatchRecords:delete'
            })
          } else {
            this.showMessage(responseData)
          }
    ...
    onJsonView() {
      ...
      this.$refs.jsonView.$emit('openViewDialog', checkBoxData[0].id)
    },
    onView() {
      let checkBoxData = this.$refs.table.getCheckboxRecords();
      if (checkBoxData.length!=1){
      ...
      // console.log(this.$refs.table.getRadioReserveRecord())
      this.$refs.PushIndex.$emit('openViewDialog', checkBoxData[0].id)
    },
    ...
    onDelete(scope) {
      this.$confirm('确定删除吗？', '确认', {
        ...
        .then(() => {
          this.setLoad()
          deleteMasterDataBatchRecords(scope.row)
            .then((responseData) => {
              if (responseData.code == 100) {
                this.getList()
                this.showMessage({
