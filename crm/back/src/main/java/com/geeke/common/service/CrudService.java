...
public abstract class CrudService<D extends CrudDao<T>, T extends DataEntity<T>> extends BaseService {
	...
	public List<T> getCrmddfzrs(String loginName){return dao.getCrmddfzrs(loginName);};


	public List<T>  getBycusnames(String id) {
		return dao.getBycusnames(id);
	}
	...
	public List<T> getBycompany(String loginName){
		return dao.getBycompany(loginName);
	};
	...
	protected int doDelete(T entity) {
		...
		System.out.println(delete+"参数1111"+entity.getId());
		if (delete != 1) {
			throw new CommonJsonException(ResultUtil.warningJson(ErrorEnum.E_50002, "该记录非最新版本，操作失败！"));
		}
		return delete;
