<template>
  <el-row class="dc-container" v-loading="loading">
      ...
      <el-row>
        <el-col :span="8">
          <el-form-item prop="customerId" label="客户名称" :rules="[{ required: true, message: '客户名称不能为空', trigger: 'blur' }]"
            ...
            <el-select clearable v-model="editFormData.customerId" placeholder="请选择客户名称" style="width: 100%;"
        ...
        <el-col :span="8">
          <el-form-item prop="opportunityId" label="商机" :rules="[{ required: true, message: '商机不能为空', trigger: 'blur' }]"
            ...
            <el-select clearable v-model="editFormData.opportunityId" placeholder="请选择商机" style="width: 100%;"
              :disabled="!editFormData.customerId">
    ...
    <div style="margin-bottom:10px">
      <el-button icon="el-icon-plus" @click="addWork" :disabled="action == 'view'">新增</el-button>
    </div>
    <el-form ref="editFormData" :model="editFormData" class="dc-el-form_ElEditForm">
      <ux-grid :data="editFormData.payList" ref="tsmWorkExperienceTable" height="200px" border
        ...
        <ux-table-column field="order" title="期数" tree-node resizable min-width="100px"
          :edit-render="{ autofocus: '.el-input__inner' }" header-align="center" align="center" show-overflow
        ...
        <ux-table-column field="payAmount" title="还款金额" tree-node resizable min-width="120px" align="center"
          ...
          <template v-slot="{ row }">
            <div style="width: 100%;text-align: right;"> {{ row.payAmount }}</div>
          </template>
        </ux-table-column>
...
export default {
  ...
  methods: {
      ...
      getCrmContractByTaskId(taskId)
        .then((responseData) => {
          if (responseData.code == 100) {
            ...
            if (this.editFormData.customerId) {
              this.getBusiness(this.editFormData.customerId)
            }
    ...
    changeCustomer(val) {
      this.editFormData.opportunityId = ''
      if (val) {
