...
import { listMasterDataJsonPage, deleteMasterDataJson } from '@/api/databus/masterDataJson'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [],
      tableId: '1562488744152310002',
      schemeId: '1562488744152310000'
    }
  },
  computed: {},
  methods: {
    getList() {
      ...
      // 数据权限: Jsonmaster_data_json
      this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
      listMasterDataJsonPage(this.search)
        .then((responseData) => {
        ...
        .then((responseData) => {
          if (responseData.code == 100) {
            this.permission.view = responseData.data.find((item) => {
              return item.permission === 'masterDataJson:read'
            })
            this.permission.export = responseData.data.find((item) => {
              return item.permission === 'masterDataJson:export'
            })

            this.permission.add = responseData.data.find((item) => {
              return item.permission === 'masterDataJson:create'
            })
            this.permission.edit = responseData.data.find((item) => {
              return item.permission === 'masterDataJson:update'
            })
            this.permission.remove = responseData.data.find((item) => {
              return item.permission === 'masterDataJson:delete'
            })
          } else {
            this.showMessage(responseData)
          }
    ...
    onDelete(scope) {
      this.$confirm('确定删除吗？', '确认', {
        ...
        .then(() => {
          this.setLoad()
          deleteMasterDataJson(scope.row)
            .then((responseData) => {
              if (responseData.code == 100) {
                this.getList()
                this.showMessage({
