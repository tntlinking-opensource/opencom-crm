...
import { listProductionRelationPage, deleteProductionRelation } from '@/api/databus/productionRelation'
...
export default {
  ...
  data() {
    return {
      ...
      searchList: [],
      tableId: '1436276714930348034',
      schemeId: '1436276714930348032'
    }
  },
  computed: {},
  methods: {
    getList() {
      ...
      // 数据权限: 生产消费关系production_relation
      this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
      listProductionRelationPage(this.search)
        .then((responseData) => {
    ...
    async pageInit() {
      ...
      try {
        ...
        // 数据权限: 生产消费关系production_relation
        this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
        let [listProductionRelationRespData, listPermissionRespData] = await Promise.all([
          listProductionRelationPage(this.search),
          listResourcePermission(this.$route.meta.routerId)
        ])
        if (listProductionRelationRespData.code == 100 && listPermissionRespData.code == 100) {
          this.tableDataTotal = listProductionRelationRespData.data.total
          this.tableData = listProductionRelationRespData.data.rows ? listProductionRelationRespData.data.rows : []
          this.permission.view = listPermissionRespData.data.find((item) => {
            return item.permission === 'productionRelation:read'
          })
          this.permission.export = listPermissionRespData.data.find((item) => {
            return item.permission === 'productionRelation:export'
          })

          this.permission.add = listPermissionRespData.data.find((item) => {
            return item.permission === 'productionRelation:create'
          })
          this.permission.edit = listPermissionRespData.data.find((item) => {
            return item.permission === 'productionRelation:update'
          })
          this.permission.remove = listPermissionRespData.data.find((item) => {
            return item.permission === 'productionRelation:delete'
          })
        } else {
          this.showMessage(listPermissionRespData.code != 100 ? listPermissionRespData : listProductionRelationRespData)
        }
        this.resetLoad()
      } catch (error) {
        this.outputError(error)
    ...
    onDelete(scope) {
      this.$confirm('确定删除吗？', '确认', {
        ...
        .then(() => {
          this.setLoad()
          deleteProductionRelation(scope.row)
            .then((responseData) => {
              if (responseData.code == 100) {
                this.getList()
                this.showMessage({
