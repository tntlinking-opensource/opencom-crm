...
import {saveCrmOpportunity, sendPayQuotation} from "../../../api/crmBusiness/crmOpportunity";
...
export default {
  ...
  methods: {
    ...
    editCrmClues() {
      ...
      if (this.queryFormData.fileList.uploads.length === 0) {
        this.$message.warning("请选择文件");
        return false;
      }
      this.setLoad()
      this.queryFormData.payList = this.multipleSelection3
      this.queryFormData.opportunityId = this.rowData.id
      debugger
      ...
      this.$refs["queryForm"].validate((valid) => {
        if (valid){
          this.$confirm('确认发送报价单？', '提示', {
            ...
            sendPayQuotation(formData).then(responseData => {
