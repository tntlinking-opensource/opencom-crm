        ...
        >
          <el-row>
            ...
            <el-col :span="12">
              ...
              >
                <el-input
                  v-model="editFormData.pdClassifyName"
                  :maxLength="255"
                  placeholder="请输入产品分类"
                  clearable
                  disabled
...
/** 根据用户界面配置import组件 结束 */
import { saveCrmCustomerProducts } from '@/api/crmCustomer/crmCustomerProducts'
import { getCrmProductsById, saveCrmProducts } from '@/api/crmProInfo/crmProducts'
import BaseUI from '@/views/components/baseUI'
import OperationIcon from '@/components/OperationIcon'
export default {
  extends: BaseUI,
  name: 'crmCustomerProducts-form',
  components: {
    /** 根据用户界面配置组件 开始 */
  ...
  methods: {
      ...
      for (let i = 0; i < this.parentOptions.length; i++) {
        if (value==this.parentOptions[i].id){
          this.editFormData = {
            ...
            productsId: this.parentOptions[i].id,
            pdClassifyId: this.parentOptions[i].pdClassify.id,
            pdClassifyName: this.parentOptions[i].pdClassify.pdcName
          }
          // this.editFormData.pdNo= this.parentOptions[i].pdNo
          // this.editFormData.pdName= this.parentOptions[i].pdName
          // this.editFormData.productsId= this.parentOptions[i].id
          // this.editFormData.pdClassifyId= this.parentOptions[i].pdClassify.id
          // this.editFormData.pdClassifyName= this.parentOptions[i].pdClassify.pdcName
    ...
    doSave() {
      ...
      saveCrmCustomerProducts({
        cusId: this.cusId,
        crmProducts: this.editFormData
      })
        .then((responseData) => {
  ...
  mounted: function () {
    this.$nextTick(() => {
      this.$on('openViewDialog', async function (id) {
        ...
        const data = await this.getFormById(id)
        this.editFormData = {
          ...this.initEditData(),
          ...data,
          pdClassifyId: data.pdClassify.id,
          pdClassifyName: data.pdClassify.pdcName
        }
        this.initOptions(this.editFormData)
      ...
      this.$on('openEditDialog', async function (id) {
        ...
        this.editFormData = {
          ...this.initEditData(),
          ...data1,
          pdClassifyId: data1.pdClassify.id,
          pdClassifyName: data1.pdClassify.pdcName
        }
        this.initOptions(this.editFormData)
        this.dialogProps.visible = true
        // this.editFormData.productsId=this.editFormData.id
